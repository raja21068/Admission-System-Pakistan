 ...
 * @author Yougeshwar
 */
public class MasterSelectionListFilter extends javax.swing.JPanel implements ReportFilterProvider {

    public MasterSelectionListFilter() {
        initComponents();
    ...
    public JRBeanCollectionDataSource getJRDataSource() {
        ...
        ProgramType pt = DatabaseManager.getSingleRecord(ProgramType.class, "isBachelor = false");
        ...
        List<MasterSelectionListJRBean> masterBeanList = new ArrayList<>();
        ...
        for (SelectionListView slv : dataList) {
            ...
            try {
                ...
                int gtm = (int) result.get(2).get("TOTAL_MARKS");
                float gom = (float) result.get(2).get("MARKS_OBTAINED");
                int gyr = (int) result.get(2).get("PASSING_YEAR");
                String grdGroup = (String) result.get(2).get("GROUP_NAME");
                ...
                bean.setGrdObtained((int)gom);
                bean.setGrdYear(gyr);

                bean.setMatricPer(mom / mtm * 100.0F * IConstant.M_MATRIC_PER);
                bean.setInterPer(iom / itm * 100.0F * IConstant.M_INTER_PER);
                bean.setGrdPer((gom-slv.getDeductionMarks()) / gtm * 100.0F * IConstant.DEGREE_PER);
        ...
        for (Map.Entry<String, List<MasterSelectionListDetailJRBean>> entry : map.entrySet()) {
            ...
            bean.setDiscipline(ProgramTitleHandler.handleMaster(array[4], array[2], array[1]));
            bean.setCampusLocation(array[3]);
            bean.setCandidatesList(new JRBeanCollectionDataSource(list));
            
            masterBeanList.add(bean);
        }
        source = new JRBeanCollectionDataSource(masterBeanList);
